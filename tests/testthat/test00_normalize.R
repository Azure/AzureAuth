...
{
    expect_silent(verify_v2_scope("https://resource.com/.default"))

    # supported OpenID scope
    expect_silent(verify_v2_scope("offline_access"))

    # unsupported OpenID scope
    expect_error(verify_v2_scope("address"))

    # no scope path
    expect_warning(newscope <- verify_v2_scope("https://resource"))
    expect_equal(newscope, "https://resource/.default")
    expect_warning(newscope <- verify_v2_scope("https://resource/"))
    expect_equal(newscope, "https://resource/.default")

    # GUIDs
    expect_silent(verify_v2_scope("12345678901234567890123456789012/.default"))
    expect_warning(newscope <- verify_v2_scope("12345678901234567890123456789012"))
    expect_equal(newscope, "12345678901234567890123456789012/.default")
    expect_warning(newscope <- verify_v2_scope("12345678901234567890123456789012/"))
    expect_equal(newscope, "12345678901234567890123456789012/.default")

    # not a URI or GUID
    expect_error(verify_v2_scope("resource"))
    expect_error(verify_v2_scope("resource/.default"))
})
